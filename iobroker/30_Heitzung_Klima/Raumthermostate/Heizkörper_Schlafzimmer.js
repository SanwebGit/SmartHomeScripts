...
const CONFIG = {
    ...
    basisRegelung: {
        ...
        heizkurvenfaktor: 0.10,      // Reduziert von 0.25 für sanftere Regelung
        luftfeuchteOptimal: 50.0,    
...
async function main() {
    try {
        ...
        const basisSollTemp = neueSollTemp; // Dies ist die reine Basis (z.B. 18.0)

        // --- 4. LOGIK: Dynamische Anpassungen & Module ---
        if (!istSonderfall) {
            
            // --- 4.1 Feuchtekorrektur (Bleibt direkt additiv, da Luftqualität) ---
            if (feuchteSensorOK) {
                neueSollTemp += (CONFIG.basisRegelung.luftfeuchteOptimal - states.feuchteSensor) * CONFIG.basisRegelung.feuchteKorrekturfaktor;
            }
            
            // --- 4.2 Wetter-Analyse (Solar/Wind) - bleibt additiv, da externer Einfluss ---
            ...
            // --- 4.3 PHYSIK-MODULE & WETTERFÜHRUNG (V6.10: ALLES IM WETTBEWERB) ---
            ...
            if (aussenSensorOK && states.aussenTempSensor < CONFIG.basisRegelung.aussenTempNeutral) {
                aufschlagWetter = (CONFIG.basisRegelung.aussenTempNeutral - states.aussenTempSensor) * CONFIG.basisRegelung.heizkurvenfaktor;
            }
            ...
            // Wir nutzen hier 'basisSollTemp' für die Berechnung, um Übersteuerung zu vermeiden
            ...
            // ENTSCHEIDUNG: Wer bietet mehr? Das Wetter (Theorie) oder die Wand (Praxis)?
            // Wir nehmen das Maximum aus allen thermischen Anforderungen.
            const finalerPhysikAufschlag = Math.max(aufschlagWetter, aufschlagSchimmel, aufschlagBehaglichkeit, aufschlagHeizlast);
            
            // Anwenden
            if (finalerPhysikAufschlag > 0) {
                neueSollTemp += finalerPhysikAufschlag;
                
                // Logging mit allen Details
        ...
        for (const thermostatId of CONFIG.devices.thermostate) {
            ...
            if (CONFIG.debugLogAktiv) {
                ...
                if (CONFIG.tuerSensorNutzen) { details.push(`TuerZu=${isDoorPhysicallyClosed}`); }
...
function triggerCalculation(isHighPriority) {
    ...
    if (isHighPriority) {
        ...
        debounceTimerHighPriority = setTimeout(async () => {
            if (CONFIG.debugLogAktiv) log(`[${CONFIG.roomName}] Debounce (${priorityText}): Zeit abgelaufen, führe Hauptfunktion aus...`, 'info');
            await main();
        ...
        if (debounceTimerHighPriority) {
            if (CONFIG.debugLogAktiv) log(`[${CONFIG.roomName}] Debounce (${priorityText}): Trigger ignoriert, da Hoch-Prioritäts-Timer aktiv ist.`, 'info');
            return;
        }
        if (debounceTimerLowPriority) clearTimeout(debounceTimerLowPriority);
    ...
    if (CONFIG.debugLogAktiv) log(`[${CONFIG.roomName}] Debounce (${priorityText}): Trigger erhalten, starte ${delay} ms Timer.`, 'info');
}
// =====================================================================================
